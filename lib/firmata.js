...
SYSEX_RESPONSE[CAPABILITY_RESPONSE] = function(board) {
  ...
  var modes = Object.keys(board.MODES).map(function(key) {
    modeNames[board.MODES[key]] = key.toLowerCase(); 
  ...
  if (!board.pins.length) {
    ...
    for (var i = 2, n = 0; i < board.currentBuffer.length - 1; i++) {
      ...
      if (n === 0) {
        ...
        var mode = modeNames[board.currentBuffer[i]];
        if (board.PIN_LABELS[mode]) {
          pin[mode + "Label"] = board.PIN_LABELS[mode][board.currentBuffer[i + 1]];
        } else {
          pin[mode + "Resolution"] = board.currentBuffer[i + 1];
        }
      }
      n ^= 1;
    }
...
function Board(port, options, callback) {
  ...
  // map pin resolution to a label for serial and eventually i2c
  this.PIN_LABELS = {
