...
Board.prototype.reportAnalogPin = function(pin, value) {
  var isAnalogIndex = this.analogPins.indexOf(pin) > -1;
  var pIndex = isAnalogIndex ? pin : this.analogPins[pin];
  var oIndex = this.analogPins[0];
  var rIndex = -1;
  ...
  rIndex = this.reporting.indexOf(pin);
  isReporting = rIndex > -1;

  if (value === 0 || value === 1) {
    if (isReporting && value === 0) {
      isUpdateable = true;
      this.reporting.splice(rIndex, 1);
    }

    if (!isReporting && value === 1) {
      isUpdateable = true;
      this.reporting.push(pin);
    }

    if (isUpdateable) {
      this.pins[pIndex].report = value;
...
Board.prototype.reportDigitalPin = function(pin, value) {
  var port = REPORT_DIGITAL | (pin / 8 | 0);
  var rIndex = this.reporting.indexOf(port);
  var isReporting = rIndex > -1;
  var isUpdateable = false;
  ...
  if (value === 0 || value === 1) {
    ...
    if (!isReporting && value === 1) {
      isUpdateable = true;
      this.reporting.push(port);
    }

    if (isUpdateable) {
      this.pins[pin].report = value;
      this.sp.write(new Buffer([port, value]));
    }
  }
};
...
Board.prototype.pinMode = function(pin, mode) {
  var isAnalogIndex = this.analogPins.indexOf(pin) > -1;
  var isAnalog = mode === this.MODES.ANALOG;
  var isInput = mode === this.MODES.INPUT || isAnalog;
  var reporting = isInput ? 1 : 0;
  var method = isAnalog ? "reportAnalogPin" : "reportDigitalPin";
  var firstIndex = this.analogPins[0];

  // mode is ANALOG input, but the pin number provided
  // is 0-(whatever number of analog pins)
  if (isAnalog && !isAnalogIndex) {
    pin += firstIndex;
  }

  if (isInput) {
    this[method](pin, reporting);
  }
