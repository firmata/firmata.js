...
SYSEX_RESPONSE[CAPABILITY_RESPONSE] = function (board) {
    ...
    //commenting because digital reporting has never been on by default and might be breaking change
    //highly recommend you DONT digital report on such pins as Serial.. etc. But this is what the previous
    //code was trying to do
    // //look up port for last pin
    // var pin = board.pins.length - 1;
    // var port = Math.floor(pin / 8);

    // //turn on digital reporting for pins on all ports
    // for(var i =0; i<=port; i++){
    //     board.sp.write([REPORT_DIGITAL | i, 255]);
    // }
...
/**
 * Set reporting on pin - not very efficient, see reportDigitalPort
 * @param {number} pin The port to turn on/off reporting
 * @param {number} value Binary value 0/1 to turn reporting on/off
 */
...
Board.prototype.reportDigitalPin = function (pin, value) {
    if(value === 0 || value === 1) {
        ...
        var _pin = pin % 8;
        var _port = Math.floor(pin / 8);
        
        //set the internal state
        this.pins[pin].report = value;

        //go get the new internal state and send it
        this._getPortValue(_port, function(portValue){

            self.sp.write(new Buffer([REPORT_DIGITAL | _port, portValue]));
...
/**
 * Get the value for a given port, used by reportDigitalPin
 * @param {number} port The port to retreive from internal state
 * @param callback has one param, portValue, the 8 bit value representation of the 8pins on that port
 */

Board.prototype._getPortValue = function (port, callback) {

    //use our internal mapping to generate a port
    var firstPin = port * 8;

    var result = 0;
    for (var i = 0; i<8; i++ )
    {   
        result = result + ((this.pins[firstPin +i].report)<<i);
