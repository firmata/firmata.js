 ...
 * Handles a PING
 */
SYSEX_RESPONSE[0x67] = function(board){
  board.emit('ping');  
};
/**
...
var Board = function(port, callback) {
        ...
        this.firmware = {};
        this.currentBuffer = [];
        this.SYSEX_RESPONSE = SYSEX_RESPONSE;
...
/**
 * Pings board to see if its down
 *
 */
Board.prototype.ping = function(cb){
...
/**
 * Wrapper around the tcp connection to make it work with arduino
 */
var TCP = function(connection){
  var self = this;
    ...
    this.connection.on('data',function(data){
        if(data.length > 1){
          Array.prototype.forEach.call(data,function(value,index){
...
TCP.prototype.write = function(byte){
    if(byte.length){
        this.connection.write(new Buffer(byte));    
    } else {
        console.log(byte);
        var result = this.connection.write(new Buffer([byte]));
    }
};
/**
 *  Wrapper around server to emit connection events with a board object
 */
var FirmataServer = function(server,listenerCallback){
    var self = this;
    this.server = server;
    this.server.on('connection',function(c){
        var tcp = new TCP(c);
        var board = new Board(tcp,function(){
            listenerCallback(board);            
...
util.inherits(FirmataServer,events.EventEmitter);
FirmataServer.prototype.listen = function(port,listernerCallback){
    this.server.listen(port,listernerCallback);
};
var createServer = function(listenerCallback){
  var server = net.createServer();
  var firmataServer = new FirmataServer(server,listenerCallback);
  return firmataServer;
};
module.exports = {
    Board : Board
