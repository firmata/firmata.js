...
/**
 * Initialize the specified SPI bus.
 * @param {number} channel The bus/channel to release: 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 */

Board.prototype.spiBegin = function(channel) {
...
/**
 * Initialize the SPI bus configured for a specific device.
 * @param {number} deviceId. Each SPI device connected to a board must have a unique ID.
 *     The range of IDs is 0-31.
 * @param {object} options Options:
 *     channel {number} 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 *     bitOrder {number} SPI_BIT_ORDER.LSBFIRST (default) or SPI_BIT_ORDER.MSBFIRST 
 *     dataMode {number} SPI_DATA_MODES.SPI_MODE0 (default), SPI_DATA_MODES.SPI_MODE1,
 *         SPI_DATA_MODES.SPI_MODE2 or SPI_DATA_MODES.SPI_MODE3
 *     maxClockSpeed {number} The maximum speed of communication. For a chip rated up to
 *         20Mhz, use 20000000.
 *     wordSize {number} The number of bits in each word (default = 8 bits). Arduino currently
 *         only supports 8-bit words for SPI. 0 = 8 bits, 1 = 1 bit, 2 = 2 bits, etc.
 *     csPin {number} [optional] The chip select pin
 *         (if not specified, user must control manually using digitalWrite)
 *     csActiveState {number} [optional] SPI_CS_ACTIVE_STATE.LOW or SPI_CS_ACTIVE_STATE.HIGH
 */
...
Board.prototype.spiBeginTransaction = function(deviceId, options) {
  ...
  var bitOrder = 0;
  var dataMode = 0;
  var maxClockSpeed = 5000000; // default = 5 Mhz
...
/**
 * End the transaction, allowing other devices to use the SPI bus.
 * @param {number} channel 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 */
...
/**
 * Transfer an array of bytes to the active device on the specified SPI bus. A byte is
 * returned for each byte sent.
 * @param {Array|number} inBytes An array of bytes to transfer, or a single number if
 *     transferring only 1 byte.
 * @param {object} options Options:
 *     channel {number} 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 *     pinControl {number} This value specifies the behavior of the CS pin. The default
 *         behavior is to set the CS active state before the write and toggle at the
 *         end of the transfer, write or read sequence. To send data where the CS active
 *         state must be maintained across several calls to spiTransfer, spiWrite or spiRead,
 *         you can frame a sequence by setting SPI_CS_PIN_CONTROL.START_ONLY when sending the
 *         first transfer/write/read message. Then pass SPI_CS_PIN_CONTROL.DISABLE for all
 *         of the following transfer/write/read messages until the last message. Set 
 *         SPI_CS_PIN_CONTROL.END_ONLY for the last message to toggle the CS state at the end.
 * @param {function} callback A function to be called when the requested bytes have
 *     been received.
 */

Board.prototype.spiTransfer = function(inBytes, options, callback) {
  var channel = 0;
  var pinControl = 0;

  if (typeof inBytes === "undefined") {
    throw new Error("Must pass either a single byte or a multi-byte array to spiTransfer.");
  }
  ...
  if (!Array.isArray(inBytes)) {
    if (inBytes < 256) {
      ...
      throw new Error("spiTransfer: inBytes param must be an array of 8-bit values");
    }
  }
  ...
  if (arguments.length === 2 && typeof options === "function") {
    ...
    channel = options.channel || 0;
    pinControl = options.pinControl || 0;
  }
...
/**
 * Write an array of bytes to the active device on the specified SPI bus. No bytes
 * will be returned.
 * @param {Array|number} inBytes `An array of bytes to write, or a single number if writing
 *     only 1 byte.`
 * @param {object} options Options:
 *     channel {number} 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 *     pinControl {number} This value specifies the behavior of the CS pin. The default
...
Board.prototype.spiWrite = function(inBytes, options) {
  var channel = 0;
  var pinControl = 0;

  if (typeof inBytes === "undefined") {
    throw new Error("Must pass either a single byte or a multi-byte array to spiWrite.");
  }
  ...
  if (!Array.isArray(inBytes)) {
    if (inBytes < 256) {
      ...
      throw new Error("spiWrite: inBytes param must be an array of 8-bit values");
    }
  }
...
/**
 * Read an array of bytes from the active device on the specified SPI bus.
 * @param {Array} numBytesToRead The number of bytes expected from the SPI device.
 * @param {object} options Options:
 *     channel {number} 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 *     pinControl {number} This value specifies the behavior of the CS pin. The default
 *         behavior is to set the CS active state before the write and toggle at the
...
Board.prototype.spiRead = function(numBytesToRead, options, callback) {
  var channel = 0;
  var pinControl = 0;
...
/**
 * Convenience method to write a single value to a register.
 * 
 * @param {Array|number} register If the register is 8-bits, the register value as a number,
 *     else the register value as an array of bytes, one for each register byte.
 * @param {Array|number} value If the value is 8-bits, pass a Number, if the value is > 8 bits
 *     break up the value into a 8-bit chunks and pass as an array.
 */

Board.prototype.spiWriteRegister = function(register, value) {
  if (!Array.isArray(register)) {
    if (register < 256) {
      register = [register];
    } else {
      throw new Error("spiWriteRegister: register param must be an array of 8-bit values");
    }
  }
  if (!Array.isArray(value)) {
    ...
    value = [value];
      ...
      throw new Error("spiWriteRegister: value param must be an array of 8-bit values");
    }
  }
  this.spiWrite(register.concat(value));
};

/**
 * Convenience method to write to a register and then read its corresponding value.
 * 
 * @param {Array|number} register If the register is 8-bits, the register value as a number,
 *     else the register value as an array of bytes, one for each register byte.
...
Board.prototype.spiReadRegister = function(register, numBytesToRead, callback) {
  this.spiWrite(register, {pinControl: this.SPI_CS_PIN_CONTROL.START_ONLY});
  this.spiRead(numBytesToRead, this.SPI_CS_PIN_CONTROL.END_ONLY, callback);
};

/**
 * Release the specified SPI bus.
 * @param {number} channel The bus/channel to release: 0 = SPI (default), 1 = SPI1, 2 = SPI2, etc
 */
