...
SYSEX_RESPONSE[ACCELSTEPPER] = function(board) {
  ...
  if (command === 0x06) {
    var value = Board.decode32BitSignedInteger(board.currentBuffer.slice(4, 9));
 ...
 *    {number} stepType: One of this.STEPPER.STEPTYPE.*
...
Board.prototype.accelStepperConfig = function(opts) {
  ...
  if (typeof opts.stepType === "undefined") {
    opts.stepType = this.STEPPER.STEPTYPE.WHOLE;
  } 
  ...
  if (Array.isArray(opts.invertPins)) {
    if (opts.invertPins.indexOf(opts.motorPin1) != -1) pinsToInvert |= 0x01;
    if (opts.invertPins.indexOf(opts.motorPin2) != -1) pinsToInvert |= 0x02;
    if (opts.invertPins.indexOf(opts.motorPin3) != -1) pinsToInvert |= 0x04;
    if (opts.invertPins.indexOf(opts.motorPin4) != -1) pinsToInvert |= 0x08;
    if (opts.invertPins.indexOf(opts.enablePin) != -1) pinsToInvert |= 0x10;
  }

  data.push(pinsToInvert);
  data.push(END_SYSEX);
...
Board.prototype.accelStepperStep = function(deviceNum, steps, callback) {
  ...
  Array.prototype.push.apply(data, Board.encode32BitSignedInteger(steps));
  data.push(END_SYSEX);
...
Board.prototype.accelStepperTo = function(deviceNum, position, callback) {
  ...
  Array.prototype.push.apply(data, Board.encode32BitSignedInteger(position));
  data.push(END_SYSEX);
...
Board.prototype.accelStepperAcceleration = function(deviceNum, acceleration) {
  ...
  Array.prototype.push.apply(data, Board.encodeCustomFloat(acceleration));
  data.push(END_SYSEX);
...
Board.prototype.accelStepperSpeed = function(deviceNum, speed) {
  ...
  Array.prototype.push.apply(data, Board.encodeCustomFloat(speed));
  data.push(END_SYSEX);
...
Board.encode32BitSignedInteger = function(data) {
  var encoded = [];

  var negative = data < 0;
  data = Math.abs(data);

  encoded.push(data & 0x7F);
  encoded.push((data >> 7) & 0x7F);
  encoded.push((data >> 14) & 0x7F);
  encoded.push((data >> 21) & 0x7F);
  encoded.push((data >> 28) & 0x07);
...
Board.decode32BitSignedInteger = function(bytes) {
...
Board.encodeCustomFloat = function(input) {
    var encoded = [],
        exponent = 0,
        maxSignificand = Math.pow(2, 23),
        sign = input < 0 ? 1 : 0;
    ...
    while( !Number.isInteger(input) && input < maxSignificand ) {
        exponent -= 1;
        input *= 10;
    }

    // Reduce precision if necessary
    while( input > maxSignificand ) {
        exponent += 1;
        input /= 10;
    }
...
Board.decodeCustomFloat = function(input) {
