...
try {
  if (process.browser) {
    ...
    // } catch (error) {
    //   SerialPort = require("../test/MockSerialPort").SerialPort;
    // }
  }
} catch (err) {
  SerialPort = null;
...
SYSEX_RESPONSE[SERIAL_MESSAGE] = function(board) {
  ...
  if (command === (SERIAL_REPLY)) {
    for (var i = 3, len = board.currentBuffer.length; i < len - 1; i += 2) {
      // better to send one byte at a time or entire buffer?
      // currently sending just one byte at a time
      data = (board.currentBuffer[i + 1] << 7) | board.currentBuffer[i];
      board.emit("serial-data", {data: data, portId: portId});
    }
  }
};
...
Board.prototype.serialConfig = function(portId, baud, bytesToRead, rxPin, txPin) {
  var softSerialPins = {};
  var data = [
    START_SYSEX,
    SERIAL_MESSAGE,
 ...
 * {number} portId The serial port to start reading continuously.
 */

Board.prototype.serialStartReading = function(portId) {
  var data = [
    START_SYSEX,
    SERIAL_MESSAGE,
    SERIAL_READ | portId,
 ...
 * {number} portId The serial port to stop reading.
 */

Board.prototype.serialStopReading = function(portId) {
  var data = [
    START_SYSEX,
    SERIAL_MESSAGE,
    SERIAL_READ | portId,
 ...
 * {number} portId The serial port to close.
 */
 ...
 * data.
 * {number} portId The serial port to flush.
 */
 ...
 * {number} portId The serial port to listen on.
 */
