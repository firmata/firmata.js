...
/**
 * Functions for interfacing with Pololu, Stepstick, or similar stepper motor drivers that are controlled by a direction and step pin
 * This supports stepperFirmata that is developed by Maxuino (http://www.maxuino.org/)
 * Downloads for stepperFirmata can be found at: http://www.maxuino.org/downloads
 */

/**
 * Setup a stepper that is attached via a 2 pin controllable stepper driver
 *
 * @param {number} numberStepsPerRev number of steps for the stepper to make one revolution
 * @param {number} directionPin pin that is connected to the direction pin on stepper driver
 * @param {number} stepPin pin that is connected to the step pin on stepper driver
 */
Board.prototype.stepperSetup = function(stepperNumber, numberStepsPerRev, directionPin, stepPin) {
    this.sp.write([START_SYSEX, STEPPER, 0x00, stepperNumber, numberStepsPerRev, directionPin, stepPin, END_SYSEX]);
};

/**
 * Set the speed of a stepper in revolutions per minute
 *
 * @param {number} stepperNumber index number for the stepper, values range 0-5
 * @param {number} rpm speed of the motor in rpms
 */
Board.prototype.stepperSpeed = function(stepperNumber, rpm) {
    var lowByte, highByte;
    highByte = ((rpm & 0xFF00) >> 8) & 0xFF;
    lowByte = rpm & 0xFF;
    this.sp.write([START_SYSEX, STEPPER, 0x02, stepperNumber, lowByte, highByte, END_SYSEX]);
};

/**
 * Move a stepper a number of steps
 *
 * @param {number} stepperNumber index number for the stepper, values range 0-5
 * @param {number} numberOfSteps number of steps for the motor to move, negative will move the motor in reverse direction
 */
Board.prototype.stepperStep = function(stepperNumber, numberOfSteps) {
    var negative = 0;
    if(numberOfSteps < 0) { negative = 1; }
    numberOfSteps = Math.abs(numberOfSteps);
    var lowByte, highByte;
    highByte = ((numberOfSteps & 0xFF00) >> 8) & 0xFF;
    lowByte = numberOfSteps & 0xFF;
    this.sp.write([START_SYSEX, STEPPER, 0x01, stepperNumber, lowByte, highByte, negative, END_SYSEX]);
};
