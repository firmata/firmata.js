...
var PIN_MODE = 0xF4,
    ...
    I2C_CONFIG = 0x78,
    STRING_DATA = 0x71,
    SOFTWARE_SERIAL_CONFIG = 0x01,
    SOFTWARE_SERIAL_DATA = 0x02;
...
var SoftwareSerialPort = function SoftwareSerialPort(board,rx, tx, baudrate){
    this.board = board;
    this.board.sp.write([START_SYSEX, SOFTWARE_SERIAL_CONFIG, rx, tx, END_SYSEX]);
};
SoftwareSerialPort.prototype.write = function(data){
    var bytes = [];
    bytes.push(START_SYSEX);
    bytes.push(SOFTWARE_SERIAL_DATA);
    for(var i = 0; i < data.length; i++){
        bytes.push(data.charCodeAt(i));
    }
    bytes.push(END_SYSEX);
    console.log(bytes);
    this.board.sp.write(bytes);
}
/**
 * Create Software Serial
 *
 */
Board.prototype.createSoftwareSerial = function(rx, tx, baudrate){
    return new SoftwareSerialPort(this,rx,tx,baudrate);
};
module.exports = {
    Board: Board
};