...
/**
 * Handles data received after a SHIFT_IN command
 * @private
 * @param {Board} board the current arduino board we are working with.
 */
SYSEX_RESPONSE[SHIFT_IN] = function(board){
    var data = (board.currentBuffer[2] & 0x7F) | ((board.currentBuffer[3] & 0x7F) << 7);
    board.emit('shift-in',data);
};
...
/**
 * Asks the arduino to read digital data ibce
 * @param {number} pin The pin to read data from
 * @param {function} callback The function to call when data has been received
 */

Board.prototype.digitalReadOnce = function(pin, callback) {
...
/**
 * Asks the arduino to shift data in.
 * @param {number} dataPin The pin to pass into shiftIn data pin parameter.
 * @param {number} clockPin The pin to pass into shiftIn clock pin parameter.
 * @param {number} msbFirst The value to pass into the shiftIn msbFirst parameter. 
 * @param {function} callback A function to call when we receive the data from the shiftIn call.
 */

Board.prototype.shiftIn = function(dataPin,clockPin,msbFirst,callback){
    this.sp.write([START_SYSEX,SHIFT_IN,dataPin,clockPin,msbFirst,END_SYSEX]);
    this.once('shift-in',callback);  
};

/**
 * Asks the arduino to shift data out.
 * @param {number} dataPin The pin to pass into shiftOut data pin parameter.
 * @param {number} clockPin The pin to pass into shiftOut clock pin parameter.
 * @param {number} msbFirst The value to pass into the shiftOut msbFirst parameter.
 * @param {byte} value The value to pass to the shiftOut dvalue parameter
 */

Board.prototype.shiftOut = function(dataPin,clockPin,msbFirst,value){
    this.sp.write([START_SYSEX,SHIFT_OUT,dataPin,clockPin,msbFirst,value,END_SYSEX]);
};
